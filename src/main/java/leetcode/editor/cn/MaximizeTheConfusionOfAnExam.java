...
<div><div>Related Topics</div><div><li>字符串</li><li>二分查找</li><li>前缀和</li><li>滑动窗口</li></div></div><br><div><li>👍 304</li><li>👎 0</li></div>
*/
...
class Solution {
    public int maxConsecutiveAnswers(String answerKey, int k) {
        ...
        int ans = 0, cnf = 0, cnt = 0;
        for (int l = 0, r = 0; r < chs.length; r++) {
            if (chs[r]=='T') cnt++;
            else cnf++;
            //如果F的数量大于K并且T的数量大于K，则移动窗口
            if (cnt > k && cnf > k) {
                if (chs[l++]=='T') cnt--;
                else cnf--;
            }
