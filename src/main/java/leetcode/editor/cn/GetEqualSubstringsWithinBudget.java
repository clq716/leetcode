...
<div><div>Related Topics</div><div><li>字符串</li><li>二分查找</li><li>前缀和</li><li>滑动窗口</li></div></div><br><div><li>👍 233</li><li>👎 0</li></div>
*/
...
class Solution {
    public int equalSubstring(String s, String t, int maxCost) {
        ...
        int ans = 0, sum = 0;
        for (int r = 0, l = 0; r < scs.length; r++) {
            sum += Math.abs(scs[r] - tcs[r]);
            while (sum > maxCost) {
                sum -= Math.abs(scs[l]-tcs[l]);
                l++;
            }
            //如果没有任何满足的字符，那么 l = r + 1 ， 这里计算的结果就会是0
