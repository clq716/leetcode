...
public class _2210_CountHillsAndValleysInAnArray{
    public static void main(String[] args) {
       Solution solution = new _2210_CountHillsAndValleysInAnArray().new Solution();
       solution.countHillValley(new int[]{2,4,1,1,6,5});
    }
...
class Solution {
    public int countHillValley(int[] nums) {
        ...
        for (int i = 1; i < nums.length - 1; i++) {
            ...
            if (nums[i] > nums[pre] && nums[i] > nums[i+1]) ans++;
            if (nums[i] < nums[pre] && nums[i] < nums[i+1]) ans++;
            pre = i;
        }
        return ans;
