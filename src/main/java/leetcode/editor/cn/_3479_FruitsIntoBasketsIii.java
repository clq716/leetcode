...
<div><div>Related Topics</div><div><li>线段树</li><li>数组</li><li>二分查找</li><li>有序集合</li></div></div><br><div><li>👍 50</li><li>👎 0</li></div>
*/

package leetcode.editor.cn;
public class _3479_FruitsIntoBasketsIii{
    public static void main(String[] args) {
        Solution solution = new _3479_FruitsIntoBasketsIii().new Solution();
    }
...
class Solution {
    public int numOfUnplacedFruits(int[] fruits, int[] baskets) {
        ...
        large[0] = baskets[0];
        for (int i = 1; i < n; i++) {
            large[i] = Math.max(large[i-1], baskets[i]);
        }
        for (int i = 0; i < n; i++) {
            if (find(large, fruits[i])) n--;
        }
        return ans;
    }

    private boolean find(int[] large, int fruit) {
        return false;
    }
}
