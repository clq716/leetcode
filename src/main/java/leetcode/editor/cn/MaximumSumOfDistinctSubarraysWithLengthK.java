...
<div><div>Related Topics</div><div><li>数组</li><li>哈希表</li><li>滑动窗口</li></div></div><br><div><li>👍 74</li><li>👎 0</li></div>
*/
...
class Solution {
    public long maximumSubarraySum(int[] nums, int k) {
        ...
        for (int i = 0; i < nums.length; i++) {
            ...
            if (i < k - 1) continue;
            if (cnt.size()==k) ans = Math.max(ans, sum);
            if (cnt.get(nums[i-k+1]) == 1) cnt.remove(nums[i-k+1]);
            else cnt.put(nums[i-k+1], cnt.get(nums[i-k+1])-1);
            sum-=nums[i-k+1];
        }
        return ans;
    }
