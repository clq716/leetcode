...
<div><div>Related Topics</div><div><li>数组</li><li>字符串</li><li>计数</li></div></div><br><div><li>👍 56</li><li>👎 0</li></div>
*/
...
class Solution {
    public int vowelStrings(String[] words, int left, int right) {
        boolean[] yuanyin = new boolean[26];
        yuanyin[0] = true;
        yuanyin['e'-'a'] = true;
        yuanyin['i'-'a'] = true;
        yuanyin['o'-'a'] = true;
        ...
        for (int i = left; i < right+1; i++) {
            if (yuanyin[words[i].charAt(0)-'a'] && yuanyin[words[i].charAt(words[i].length()-1)-'a']) ans++;
        }
        return ans;
    }
}
