...
<div><div>Related Topics</div><div><li>字符串</li><li>滑动窗口</li></div></div><br><div><li>👍 19</li><li>👎 0</li></div>
*/
...
public class ShortestAndLexicographicallySmallestBeautifulString{
    public static void main(String[] args) {
        Solution solution = new ShortestAndLexicographicallySmallestBeautifulString().new Solution();
        solution.shortestBeautifulSubstring("1100100101011001001", 7);
    }
    ...
    public String shortestBeautifulSubstring(String s, int k) {
        ...
        for (int l = 0, r = 0; r < chs.length; r++) {
            ...
            if (ones == k) {
                ...
                if (ans.isEmpty() || ss.length() < ans.length()) {
                    ans = ss;
                } else if ( ss.length() == ans.length()){
                    ans = ans.compareTo(ss) > 0 ? ss : ans;
                }
                ones--;
                l++;
            }
