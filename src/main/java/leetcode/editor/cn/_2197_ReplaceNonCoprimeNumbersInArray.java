...
<div><div>Related Topics</div><div><li>栈</li><li>数组</li><li>数学</li><li>数论</li></div></div><br><div><li>👍 55</li><li>👎 0</li></div>
*/
...
public class _2197_ReplaceNonCoprimeNumbersInArray{
    public static void main(String[] args) {
        Solution solution = new _2197_ReplaceNonCoprimeNumbersInArray().new Solution();
    }
    ...
    public List<Integer> replaceNonCoprimes(int[] nums) {
        ...
        for (int num : nums) {
            while (!result.isEmpty() && gcd(num, result.get(result.size()-1)) > 1) {
                num = lcm(num, result.remove(result.size()-1));
            }
            result.add(num);
        }
        return result;
