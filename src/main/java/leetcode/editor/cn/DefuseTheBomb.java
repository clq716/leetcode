...
<div><div>Related Topics</div><div><li>数组</li><li>滑动窗口</li></div></div><br><div><li>👍 181</li><li>👎 0</li></div>
*/
...
class Solution {
    public int[] decrypt(int[] code, int k) {
        ...
        if (k == 0) return ans;
        //如果K是正数，从数组开头，从右到左滑 1 -> k+1
        //如果k是负数，从数组末尾，从左到右滑 n+k -> n
        int sum = 0, n = code.length, delta = k > 0 ? -1 : 1;
        for (int i = k>0?1:n+k; i < (k>0?k+1:n); i++) sum += code[i];
        for (int i = 0; i < n; i++) {
            ans[i] = sum;
            if (i==n-1) break;
            if (k > 0) {
