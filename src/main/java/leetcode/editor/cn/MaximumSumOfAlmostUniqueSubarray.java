...
<div><div>Related Topics</div><div><li>数组</li><li>哈希表</li><li>滑动窗口</li></div></div><br><div><li>👍 55</li><li>👎 0</li></div>
*/
...
class Solution {
    public long maxSum(List<Integer> nums, int m, int k) {
        ...
        for (int i = 0; i < nums.size(); i++) {
            if (countMap.get(nums.get(i)) == null) {
                countMap.put(nums.get(i), 1);
            } else {
                countMap.put(nums.get(i), countMap.get(nums.get(i))+1);
            }
            sum += nums.get(i);
            if (i < k-1) continue;
            if (countMap.size() >= m) ans = Math.max(sum, ans);
            if (countMap.get(nums.get(i-k+1)) == 1) {
                countMap.remove(nums.get(i-k+1));
            } else {
                countMap.put(nums.get(i-k+1), countMap.get(nums.get(i-k+1))-1);
            }
            sum -= nums.get(i-k+1);
        }
        return ans;
