...
<div><div>Related Topics</div><div><li>深度优先搜索</li><li>字典树</li><li>数组</li><li>字符串</li></div></div><br><div><li>👍 189</li><li>👎 0</li></div>
*/
...
public class _1233_RemoveSubFoldersFromTheFilesystem{
    public static void main(String[] args) {
        Solution solution = new _1233_RemoveSubFoldersFromTheFilesystem().new Solution();
        solution.removeSubfolders(new String[]{"/a","/a/b","/c/d","/c/d/e","/c/f"});
    }
...
class Solution {
    public List<String> removeSubfolders(String[] folder) {
        ...
        for(String filePath : folder) {
            ...
            for (int i = 1; i < fileNames.length; i++) {
                tmp = tmp.getOrAddChild(fileNames[i]);
            }
            tmp.realName = filePath;
        }
        tree(root, ans);
        return ans;
    }

    private void tree(Node node, List<String> ans) {
        if (node.realName != null) {
            ans.add(node.realName);
    ...
    class Node {
        ...
        public Node(String name, Map<String, Node> map) {
            this.name = name;
            this.children = map;
        }
        ...
        public Node getOrAddChild(String fileName) {
            if (children == null) children = new HashMap<>();
            return children.computeIfAbsent(fileName, ignore -> new Node(fileName));
        }
    }
}
